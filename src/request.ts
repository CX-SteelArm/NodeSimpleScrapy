...
export async function run (): Promise<void> {
  let url = REQ_CONTEXT.url
  if (typeof url !== 'string' || !/^https?:/.test(url)) {
    throw new Error('can only use http(s) protocol.')
  }
  if (REQ_CONTEXT.headers) {
    REQ_CONTEXT.headers = new Headers(REQ_CONTEXT.headers)
  }
  return fetch(url, REQ_CONTEXT)
    .then(res => {
      if (res.ok) {
    ...
    .then(text => findImgsInContent(text))
